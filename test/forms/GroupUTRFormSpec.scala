...
import models.GroupUTR
import play.api.data.Form
import uk.gov.hmrc.play.test.{UnitSpec, WithFakeApplication}

class GroupUTRFormSpec extends UnitSpec with WithFakeApplication {

  def fillForm(radioButtonValue: String, deValue: Option[String]): Form[GroupUTR] = {
    GroupUtrForm.form.bind(Map("noutr" -> radioButtonValue, "utr" -> deValue.getOrElse("")))
  }

  " form" should {
    "have currentEmail as registrationEmail and transform DifferentEmail to NONE" in {
      val result = fillForm("1234567890", Some("99999999"))
println("###############################" + result)
